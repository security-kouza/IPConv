...
int same_declaration (cell * X, cell * Y, int exact){
  if(!X && !Y)                  return 1;
  if(!X)                        return 0;
  if(!Y)                        return 0;
  if(X->car == AST_PARENTHESIS) return same_declaration (X->element(2),Y,exact);
  if(Y->car == AST_PARENTHESIS) return same_declaration (X,Y->element(2),exact);
  if(X->car == AST_INIT)        return same_declaration (X->element(2),Y,exact);
  if(Y->car == AST_INIT)        return same_declaration (X,Y->element(2),exact);
  if(X->car == AST_DECLARATOR) if(X->element(2)->car == AST_NULL)
                                return same_declaration (X->element(3),Y,exact);
  if(Y->car == AST_DECLARATOR) if(Y->element(2)->car == AST_NULL)
                                return same_declaration (X,Y->element(3),exact);
  if(X->car != Y->car)          
...
//          ((X->car == AST_NEW_IDENTIFIER) && (Y->car == AST_IDENTIFIER)))) 
  ...
  for( X=X->pointer(2), Y=Y->pointer(2) ; X && Y ; X=X->cdr, Y=Y->cdr) {
    if(!same_declaration(X->element(0),Y->element(0),exact)) return 0 ;
  }
  if(X)                         return 0;
  if(Y)                         return 0;
                                return 1;
